...
suite('time.ts tests', () => {
	...
	*/
    ...
    test('WallTime :: constructor', () => {
        assert.throws(() => { new WallTime(-1, 0, 0); }, "Invalid hours -1");
        assert.throws(() => { new WallTime(0, -1, 0); }, "Invalid minutes -1");
        assert.throws(() => { new WallTime(0, 0, -1); }, "Invalid seconds -1");
        assert.doesNotThrow(() => { new WallTime(0, 0, 0); }, "Valid time 0:0:0 threw exception");
        assert.doesNotThrow(() => { new WallTime(1, 0, 0); }, "Valid time 1:0:0 threw exception");
        assert.doesNotThrow(() => { new WallTime(0, 1, 0); }, "Valid time 0:1:0 threw exception");
        assert.doesNotThrow(() => { new WallTime(0, 0, 1); }, "Valid time 0:0:1 threw exception");
        assert.doesNotThrow(() => { new WallTime(1, 1, 1); }, "Valid time 1:1:1 threw exception");

        let wt = new WallTime(1, 2, 3);
        assert.equal(wt.hours, 1, "Invalid hours");
        assert.equal(wt.minutes, 2, "Invalid minutes");
        assert.equal(wt.seconds, 3, "Invalid seconds");

        wt = new WallTime(1, 2, 60);
        assert.equal(wt.hours, 1, "Invalid hours");
        assert.equal(wt.minutes, 3, "Invalid minutes");
        assert.equal(wt.seconds, 0, "Invalid seconds");

        wt = new WallTime(1, 125, 2);
    ...
    test('WallTime :: add', () => {
        let wt1 = new WallTime(1, 2, 3);
        let wt2 = new WallTime(4, 5, 6);
        ...
        wt1 = new WallTime(23, 59, 59);
        wt2 = new WallTime(0, 0, 1);
        result = wt1.add(wt2);
        assert.equal(result.hours, 24, "Invalid hours");
    ...
    test('WallTime :: cmp', () => {
        ...
        wt1 = new WallTime(1, 2, 3);
        wt2 = new WallTime(1, 2, 3);
        result = wt1.cmp(wt2);
