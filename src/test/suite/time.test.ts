...
suite('time.ts tests', () => {
	...
	*/
    ...
    test('WallTime :: constructor', () => {
        assert.throws(() => {
            new WallTime(-1, 0, 0, 0);
        }, 'Invalid days -1');
        assert.throws(() => {
            new WallTime(0, -1, 0, 0);
        }, 'Invalid hours -1');
        assert.throws(() => {
            new WallTime(0, 0, -1, 0);
        }, 'Invalid minutes -1');
        assert.throws(() => {
            new WallTime(0, 0, 0, -1);
        }, 'Invalid seconds -1');
        assert.doesNotThrow(() => {
            new WallTime(0, 0, 0, 0);
        }, 'Valid time 0:0:0 threw exception');
        assert.doesNotThrow(() => {
            new WallTime(1, 0, 0, 0);
        }, 'Valid time 1-0:0:0 threw exception');
        assert.doesNotThrow(() => {
            new WallTime(0, 1, 0, 0);
        }, 'Valid time 1:0:0 threw exception');
        assert.doesNotThrow(() => {
            new WallTime(0, 0, 1, 0);
        }, 'Valid time 0:1:0 threw exception');
        assert.doesNotThrow(() => {
            new WallTime(0, 0, 0, 1);
        }, 'Valid time 0:0:1 threw exception');
        assert.doesNotThrow(() => {
            new WallTime(1, 1, 1, 1);
        }, 'Valid time 1:1:1 threw exception');
    ...
    test('WallTime :: fromString', () => {
        assert.throws(() => {
            WallTime.fromString('1:2:3:4');
        }, 'Invalid time string: 1:2:3:4');
        assert.throws(() => {
            WallTime.fromString('1:2:3:4:5');
        }, 'Invalid time string: 1:2:3:4:5');
        assert.throws(() => {
            WallTime.fromString('garbage');
        }, 'Invalid time string: garbage');
        assert.throws(() => {
            WallTime.fromString('a1:2:3');
        }, 'Invalid time string: a1:2:3');
        assert.throws(() => {
            WallTime.fromString('20.00.15');
        }, 'Invalid time string: 20.00.15');
        assert.throws(() => {
            WallTime.fromString('1-1-2:3');
        }, 'Invalid time string: 1-1-2:3');
        assert.throws(() => {
            WallTime.fromString('1-1:2-3');
        }, 'Invalid time string: 1-1:2-3');
        assert.throws(() => {
            WallTime.fromString('1-1-1:2:3');
        }, 'Invalid time string: 1-1-1:2:3');
        assert.doesNotThrow(() => {
            WallTime.fromString('1-1:2:3');
        }, 'Valid time string: 1-1:2:3 threw exception');
        assert.doesNotThrow(() => {
            WallTime.fromString('1');
        }, 'Valid time string: 1 threw exception');
        assert.doesNotThrow(() => {
            WallTime.fromString('1:2');
        }, 'Valid time string: 1:2 threw exception');
        assert.doesNotThrow(() => {
            WallTime.fromString('1:2:3');
        }, 'Valid time string: 1:2:3 threw exception');
